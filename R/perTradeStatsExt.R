...
perTradeStatsExt <- function(Portfolio, Symbol, includeOpenTrade=TRUE,
                             ...
                             includeInitialExitTransaction=TRUE, # TODO: find a better arg.name ----
                             Dates=NULL,
                             ...)
{
    ...
    switch(tradeDef,
           flat.to.flat = {
               # TODO: discuss whether it is correct to ignore transactions in the 'head' of posPL ----
    ...
    for(i in 1:N)
    {
        timespan <- seq.int(trades$Start[i], trades$End[i]) # TODO: add a comment to clarify this ----
