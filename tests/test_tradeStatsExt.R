...
if(1) {
    ...
    checkEquals( ts$RINA.Index                , -1.1947141204532867 ) #-0.89724388797507726 ) #-0.90635296297990031 ) # -1.6159319274181116 ) # -0.90635296297990031
...
if(1) {
    ...
    # includes the opening / initial exit transaction, i.e. counts thansactions correctly
    checkEquals( ts$Num.Trades                 , 7                            )
...
if(1) {
    ...
    dates <- paste0(pd$first.time,"::",pd$last.time)
    print(dates)
    ts <- tradeStatsExt("forex","GBPUSD", Dates = dates, debugF = TRUE, Interval = "6hours")
